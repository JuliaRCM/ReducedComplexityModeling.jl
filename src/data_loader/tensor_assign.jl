...
The function `assign_output_estimate` is closely related to the [`GeometricMachineLearning.Transformer`](@extref).
...
function augment_zeros(output_diff::Thunk, seq_length)
    Thunk(() -> augment_zeros(unthunk(output_diff), seq_length))
end
